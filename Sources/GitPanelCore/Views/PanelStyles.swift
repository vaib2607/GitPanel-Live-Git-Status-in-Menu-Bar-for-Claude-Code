...
struct DiffSummaryView: View {
    ...
    var body: some View {
                ...
                if state.linesAdded > 0 {
                    HStack(spacing: 3) {
                        ...
                        Text("\(state.linesAdded)")
                            .font(.system(size: 13, weight: .medium, design: .monospaced)) // Diff stats are monospaced
                            .monospacedDigit()
                            .foregroundStyle(.green)
                    }
                }
        ...
        .accessibilityLabel(state.hasChanges ? "\(state.linesAdded) lines added, \(state.linesDeleted) lines deleted" : "No changes")
    }
}
...
struct FileStatsView: View {
    ...
    private func statChip(count: Int, label: String, color: Color) -> some View {
        HStack(spacing: 3) {
            Text("\(count)")
                .font(.system(size: 11, weight: .semibold, design: .monospaced)) // Diff stats
                .monospacedDigit()
            Text(label)
                .font(.system(size: 11))
        }
    ...
    var body: some View {
        HStack(spacing: 4) {
            if state.isAheadOfRemote {
                ...
                .accessibilityLabel("Branch is ahead of remote")
            }
            if state.isBehindRemote {
                ...
                .accessibilityLabel("Branch is behind remote")
            }
        }
    }
}
...
struct RepoStateBadge: View {
    ...
    var body: some View {
        ...
        .foregroundStyle(state.color)
        .accessibilityElement(children: .combine)
        .accessibilityLabel("Repository status: \(state.label)")
    }
}
...
struct FooterActionsView: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            .accessibilityHint("Reloads the current repository state")
            ...
            .accessibilityHint("Fetches and integrates changes from the remote repository")
        }
    }
}
...
struct DropdownRow: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            ...
            .buttonStyle(.plain)
            .accessibilityLabel("\(title): \(value)")
            .accessibilityHint("Tap to change \(title.lowercased())")
        }
        .frame(minHeight: 32)
